...
describe Triannon::LdpWriter, :vcr do
  ...
  before(:all) do
    ...
    @root_anno_url = "#{Triannon.config[:ldp]['url']}/#{Triannon.config[:ldp]['uber_container']}"
  end
  let(:ldpw) { Triannon::LdpWriter.new @anno, 'foo' }
  let(:conn) { Faraday.new(url: @root_anno_url) }
  ...
  context "#create_base" do
    it 'LDP store creates Basic Container for the annotation and returns id' do
      ...
      full_url = "#{@root_anno_url}/#{new_pid}"
    ...
    it 'IIIF anno has sc:painting motivation' do
      ...
      g = RDF::Graph.new.from_ttl(resp.body)
      full_url = "#{@root_anno_url}/#{base_pid}"
      expect(g.query([RDF::URI.new(full_url), RDF.type, RDF::Vocab::OA.Annotation]).size).to eql 1
  ...
  context "#create_body_container" do
    ...
    it 'LDP store creates retrievable LDP DirectContainer with correct member relationships' do
      ...
      g = RDF::Graph.new.from_ttl(resp.body)
      full_url = "#{@root_anno_url}/#{id}/b"
      expect(g.query([RDF::URI.new(full_url), RDF.type, RDF::Vocab::LDP.DirectContainer]).size).to eql 1
      expect(g.query([RDF::URI.new(full_url), RDF::Vocab::LDP.membershipResource, RDF::URI.new("#{@root_anno_url}/#{id}")]).size).to eql 1
  ...
  context '#create_body_resources' do
    ...
    context "after create_resources_in_container" do
      ...
      it 'body container gets ldp:contains statement' do
        ...
        g = RDF::Graph.new.from_ttl(resp.body)
        body_cont_url = "#{@root_anno_url}/#{@anno_id}/b"
      ...
      it 'base container gets oa:hasBody statement' do
        ...
        g = RDF::Graph.new.from_ttl(resp.body)
        anno_url = "#{@root_anno_url}/#{@anno_id}"
  ...
  context '#create_target_resources' do
    ...
    context "after create_resources_in_container" do
      ...
      it 'target container gets ldp:contains statement' do
        ...
        g = RDF::Graph.new.from_ttl(resp.body)
        target_cont_url = "#{@root_anno_url}/#{@anno_id}/t"
