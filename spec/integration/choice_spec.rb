...
describe "integration tests for Choice", :vcr do
  it "body is choice of ContentAsText" do
    ...
    expect(body_subj_node_solns).to include [body_blank_node, RDF.type, RDF::OpenAnnotation.Choice]
    default_solns = g.query [body_blank_node, RDF::OpenAnnotation.default, nil]
    expect(default_solns.count).to eq 1
    default_blank_node = default_solns.first.object
    item_solns = g.query [body_blank_node, RDF::OpenAnnotation.item, nil]
    ...
    expect(body_subj_node_solns.count).to eq 3
    expect(body_subj_node_solns).to include [body_blank_node, RDF.type, RDF::OpenAnnotation.Choice]
    default_solns = h.query [body_blank_node, RDF::OpenAnnotation.default, nil]
    expect(default_solns.count).to eq 1
    default_blank_node = default_solns.first.object
    item_solns = h.query [body_blank_node, RDF::OpenAnnotation.item, nil]
    expect(item_solns.count).to eq 1
    item_blank_node = item_solns.first.object
  ...
  it "body is choice of external URIs with addl metadata" do
    ...
    write_anno = Triannon::Annotation.new data: "
     ...
     expect(g.query([body_blank_node, RDF::OpenAnnotation.default, default_uri_obj]).count).to eql 1
     default_solns = g.query [default_uri_obj, nil, nil]
     expect(default_solns.count).to eq 2
     expect(default_solns).to include [default_uri_obj, RDF.type, RDF::DCMIType.Sound]
     ...
     expect(g.query([body_blank_node, RDF::OpenAnnotation.item, item_uri_obj]).count).to eql 1
     ...
     expect(h.query([body_blank_node, RDF::OpenAnnotation.default, default_uri_obj]).count).to eql 1
     default_solns = h.query [default_uri_obj, nil, nil]
     expect(default_solns.count).to eq 2
     expect(default_solns).to include [default_uri_obj, RDF.type, RDF::DCMIType.Sound]
     expect(default_solns).to include [default_uri_obj, RDF::DC11.format, body_default_format]

     expect(h.query([body_blank_node, RDF::OpenAnnotation.item, item_uri_obj]).count).to eql 1
  ...
  it "target is choice of external URIs, default w addl metadata" do
    ...
    @prefix xsd: <http://www.w3.org/2001/XMLSchema#> .
     ...
     expect(target_subj_node_solns).to include [target_blank_node, RDF.type, RDF::OpenAnnotation.Choice]
     default_solns = g.query [target_blank_node, RDF::OpenAnnotation.default, nil]
     expect(default_solns.count).to eq 1
     default_blank_node = default_solns.first.object
     item_solns = g.query [target_blank_node, RDF::OpenAnnotation.item, nil]
     expect(item_solns.count).to eq 1
     item_blank_node = item_solns.first.object
     ...
     target_solns = h.query([nil, RDF::OpenAnnotation.hasTarget, nil])
     expect(target_solns.size).to eql 1
     target_blank_node = target_solns.first.object
     target_subj_node_solns = h.query [target_blank_node, nil, nil]
     expect(target_subj_node_solns.count).to eq 3
     expect(target_subj_node_solns).to include [target_blank_node, RDF.type, RDF::OpenAnnotation.Choice]
     default_solns = h.query [target_blank_node, RDF::OpenAnnotation.default, nil]
     expect(default_solns.count).to eq 1
     default_blank_node = default_solns.first.object
     item_solns = h.query [target_blank_node, RDF::OpenAnnotation.item, nil]
     expect(item_solns.count).to eq 1
     item_blank_node = item_solns.first.object
  ...
  it "target is choice of three images URIs" do
    ...
    @prefix xsd: <http://www.w3.org/2001/XMLSchema#> .
     ...
     default_node_subject_solns = g.query [default_blank_node, nil, nil]
     expect(default_node_subject_solns.count).to eq 1
     expect(default_node_subject_solns).to include [default_blank_node, RDF.type, RDF::DCMIType.Image]
     item1_subject_solns = g.query [item1_blank_node, nil, nil]
     expect(item1_subject_solns.count).to eq 1
     expect(item1_subject_solns).to include [item1_blank_node, RDF.type, RDF::DCMIType.Image]
